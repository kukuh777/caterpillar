  ...
  const params = {
    ...
    variables: {
      input: {
        customer_id: custId,
        brand_id: brandId,
        subtotal: parseInt(subtotalPrices),
        items,
      },
    },
  };
...
async function swiftCheckout(data = {}, isSubscribe = false) {
	if (isLoginRequired && !custId) {
        window.location.href = loginUrl;
    } else {
        const cartData = Object.keys(data).length === 0 ? await fetchCartData() : data;
        const checkoutData = await createCart(cartData);
        ...
        try { 
            ...
            window.open(`${scv2PwaUrl}/authentication?state=` + encrypted, '_self');
        } catch (e) {
            console.log('error', e);
          }
    }
                ...
                for (let i = 0; i < element.children.length; i++) {
                    if (element.children[i].textContent.indexOf(custOrder.id) !== -1) {
                        if (element.children[i].children.length === 0) {
                            ...
                            const custOrdersData = custOrders.find(({ id }) => id === custOrder.id);
                            if (custOrdersData.fulfillmentStatus === 'unfulfilled' && custOrdersData.fulfillmentStatusLabel === 'Pending') {
                                let urlPaymentRetry;
                                const paymentRetryData = await paymentRetry();
...
/* Override Checkout */
{
    ...
    function getCheckoutButton() {
        ...
        if (checkoutButton && checkoutButton.length > 0) {
            ...
            lastCheckoutButton.classList.add('scv2-checkout-button');
    ...
    */
    ...
    if (paymentButton) {
        ...
        if (buynowButton && formContainer) {
            ...
            buttonClone.id = "scv2-checkout-button"
...

{
    ...
    //       const fetchProduct = await fetchProductData();
    ...
    //         cloneBuyNowButton.id = 'scv2-subscribe-button';
    ...
    //                 }
    //             });
    //             swiftCheckout(pdpLineItems, true);
    //         };
    //       }
